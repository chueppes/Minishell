...
int handle_errors(int type_error, int code_error, char *output)
{
    ...
    if(type_error == FORK_ERR)
        put_error("Error at ./Minishell: Fork Failed.");
    if(type_error == QUOTE_ERR)
        put_error("Error at ./Minishell: Did not found matching quote.");
    if(type_error == PIPERR_ERR)
        put_error("Error at ./Minishell: Was not able to create pipe.");
    if(type_error == OPENFILE_ERR)
        put_error("Error at ./Minishell: Syntax error near unexpected token 'newline'.");
    ...
    if(type_error == CMDNOTFOUND_ERR)
        put_error("Error at ./Minishell Command not found: ");
    put_error(output);
    ft_putstr_fd("\n", 2);
    return(1);
}